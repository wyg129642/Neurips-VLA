"""Physical Intuition Reasoning sims."""
...
def _polyline_progress(pos, pts) -> float:
    """Fraction of polyline ``pts`` that the closest point to ``pos`` covers."""
...
class SeesawSim(PhysicsReasoningSim):
    """Read the seesaw tilt to deduce the heavier block."""
    ...
    def oracle_waypoints(self):
        ...
        return [
            [h[0], h[1], h[2] + 0.06, -1.0],
            [h[0], h[1], h[2], 1.0],
            [h[0], h[1], h[2] + 0.12, 1.0],
    ...
    def naive_waypoints(self):
        """Always grab block_a regardless of tilt -> correct only ~50% of the time."""
...
# Maze geometry: a barrier on the left side at y=0 with a gap on the right.
_OUTER = 0.15
_GAP_X = 0.05
_BARRIER_Y = 0.0
_BARRIER_HY = 0.012
...
class MazeSim(PhysicsReasoningSim):
    """Detour through the right-side gap; a straight line hits the barrier."""

    category = "physical"

    def _setup(self) -> None:
        r = self.rng
        mirror = float(r.choice([-1.0, 1.0]))
        self._mirror = mirror
        self._start = np.array([-0.10 * mirror, 0.11, _Z])
        self._tgt = np.array([-0.10 * mirror, -0.11, _Z])
        self._gap = np.array([0.10, 0.0, _Z])
    ...
    def naive_waypoints(self):
        """Straight line from ball to target; blocked by the central wall."""
