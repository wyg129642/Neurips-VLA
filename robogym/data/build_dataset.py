"""CLI: build the balanced expert dataset (paper §4.1).

    python -m robogym.data.build_dataset --demos-per-task 500 --out datasets/robogym_expert
    python -m robogym.data.build_dataset --quick          # tiny smoke build

Streams each task's success-filtered demos straight to disk in the LIBERO HDF5 schema (+ RLDS shards + manifest), so the full 25k set never has
to live in memory.
"""
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--rlds", action="store_true",
                    help="also emit RLDS/TFDS-style shards")
    ap.add_argument("--quick", action="store_true",
                    help="smoke build: 3 demos/task")
    ...
    print(f"Building expert dataset -> {out}  "
          f"(target {cfg.demos_per_task} demos/task × 50 tasks "
    ...
    print(f"\nTotal demos: {final_report.total_demos} | "
          f"per-domain: {final_report.per_category} | "
