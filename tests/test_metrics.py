"""Metric-engine tests: trajectory evaluator behaviour + canonical formulas."""
...
def test_oracle_replay_scores_high():
    """Replaying the expert demo must yield high completeness & total."""
    ...
    # jerk-free cosine-eased expert => credible smoothness & safety reference
    assert s["smoothness"] >= 50.0, s
    assert s["safety"] >= 80.0, s

def test_idle_policy_scores_low():
    """A frozen policy completes nothing -> low completeness."""
    sim = SyntheticPickPlaceSim(task_objects=["alphabet_soup"], seed=0)
    ea, eis = sim.generate_expert_demo()
...
def test_dtw_completeness_bounds():
    ...
    assert phase_completeness(expert, expert) > 0.95          # perfect
    assert phase_completeness(expert[:1], expert) < 0.30      # no progress
    assert 0.0 <= unified_progress([0.5, 0.0], [0.5, 0.5]) <= 1.0

def test_sequence_gate_blocks_later_phases():
    # phase 2 incomplete => phase 3 must not contribute
